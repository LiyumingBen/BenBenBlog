---
title: Vue源码解析
date: 2022-04-05 10:27:01
permalink: /pages/8586014dcf128/
categories:
  - vue
tags:
  - vue
...
1. 编写页面模板
   ...
   3. 使用 单文件 ( `<template />` )
2. 创建 Vue 的实例
   1. 在 Vue 的构造函数中提供: data, methods, computed, watcher, props 等
...
- Vue 使用的是虚拟 DOM
- 只考虑了单属性 `{{'name'}}`，而 Vue 中大量的使用层级 `{{'child.name.firstName'}}`
...
**虚拟 DOM 能够极大的提高页面渲染的性能，减少页面的缓存等因素的影响。**
...
在 Vue 中，要将模板和数据相结合其实需要经历两个过程，这其中也涉及到了两种算法（思路与深拷贝类似）：
